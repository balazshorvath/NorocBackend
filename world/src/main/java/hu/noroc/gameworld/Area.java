...
public class Area {
    ...
    public Area(double sideLength, int mapWidth, World world) {
        ...
        messageConsumer = new Thread(getMessageProcessor(world));

        messageConsumer.start();

    }

    protected Runnable getMessageProcessor(World world) {
        return () -> {
            Event message = null;
            while (World.isRunning()) {
                ...
                try {
                    ...
                    if (message instanceof AttackEvent
                            && ((AttackEvent) message).getDirectionalType()
                                                      .equals(DirectionalEvent.DirectionalType.ATTACK)) {
                        ...
                        if (message instanceof DirectionalEvent && finalMessage.getEntity() == EntityType.PLAYER
                                && ((DirectionalEvent) message).getDirectionalType()
                                == DirectionalEvent.DirectionalType.MOVING_TO
                                && !isInside(
                                ((DirectionalEvent) message).getX(),
                                ((DirectionalEvent) message).getY()
                        )) {
    ...
    protected void applySpell(AttackEvent event) {
        ...
        // This code isn't executed ever. NPCs are not present.
//        npcs.forEach(scriptedNPC -> {
//            // Target - Entity
    ...
    public void newPlayer(Player player) {
        try {
            ...
            this.players.forEach(player1 -> player.newEvent(new DataEvent(new PlayerCharacterResponse(
                    ...
                    player1.getCharacterClass().getStat().health,
                    player1.getCharacterClass().getStat().mana,
                    player1.getStats()
            ), player1.getId())));
